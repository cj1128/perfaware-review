...
func disassemble(buf []byte) ([]string, error) {
	...
	for !r.isEmpty() {
		...
		output = append(output, result.Assemble())
	}
...
func (j *JumpOrLoop) Assemble() string {
...
func (a *Arithmetic) Assemble() string {
	op := a.opName()

	switch a.typ {
	case Arithmetic_RegOrMemory_With_Register_To_Either:
		{
			source := a.regName(a.w == 1)
			target := a.rmName(a.w == 1)
	...
	case Arithmetic_Immediate_To_RegisterOrMemory:
		{
			...
			return fmt.Sprintf("%s %s %s, %d", op, dataType, a.rmName(a.w == 1), a.data)
		}
...
func (m *Mov) Assemble() string {
	switch m.typ {
	case Mov_RegisteryOrMemory_ToOrFrom_Register:
		{
			source := m.regName(m.w == 1)
			target := m.rmName(m.w == 1)
	...
	case Mov_Immediate_To_RegisterOrMemory:
		{
			...
			return fmt.Sprintf("mov %s, %s %d", m.rmName(m.w == 1), dataType, m.data)
		}
